void drawComptonX_ip6v3(){
  //TFile *finB=TFile::Open("/work/halla/moller12gev/ciprian/compton/comptonEic/beam18GeV_ip6V3_1e3.root","READ");
  TFile *finB=TFile::Open("/work/halla/moller12gev/ciprian/compton/comptonEic/beam5GeV_ip6V3_1e3.root","READ");
  TTree *tb=(TTree*)finB->Get("t");

  //TFile *finC=TFile::Open("/volatile/halla/moller12gev/ciprian/tmp/compton/o_e18Pz71Px71_ip6V3_VtxSmearXY0Z0_5e6.root","READ");
  //TFile *finC=TFile::Open("/volatile/halla/moller12gev/ciprian/tmp/compton/o_e18Pz95Pxm31_ip6V3_VtxSmearXY0Z0_1e6.root","READ");
  TFile *finC=TFile::Open("/volatile/halla/moller12gev/ciprian/tmp/compton/o_e5Pz99Pxm9_ip6V3_VtxSmearXY0Z0_1e6.root","READ");
  ...
  float gOffset[nDet]={2,29,49,107,127,185,204.5,235,237,241,243,261};
  ...
  for(int i=0;i<nDet;i++){
    ...
    gx[i]=new TH1F(Form("gx_%s",detNm[i].c_str()),
		   ...
		   600,-10+xPos[i]-gOffset[i],10+xPos[i]-gOffset[i]);
    ...
    nE=tc->Project(Form("gxp_%s",detNm[i].c_str()),
		   Form("%s_hits.x",detNm[i].c_str()),
		   Form("pXsec*(%s_hits.pID==22)",detNm[i].c_str()),"",nEvProc);
